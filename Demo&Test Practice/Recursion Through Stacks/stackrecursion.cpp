...
List allAnagrams(string input){
    ...
    while(!recurStack.empty()){
        ...
        for(int i = 0; i < stacktop.in_str.length(); i++){
            Pair to_push;
            string fixed = stacktop.fixed_str;
            string in = stacktop.in_str;
            swap(in[0], in[i]);
            to_push.fixed_str = fixed + in.front();
            to_push.in_str = in.erase(0,1);
            recurStack.push(to_push);
        }
    }
    return final_output;
...
// //repeat each character up to k times
List stretch(string input, int k){
    ...
    while(!recurStack.empty()){
        ...
        Pair to_push;
        for(int i = 1; i <= k; i++){
            string s(i, stacktop.in_str.front());
            to_push.fixed_str = stacktop.fixed_str + s;
            to_push.in_str = stacktop.in_str.substr(1);
            recurStack.push(to_push);
        }
    }
    return final_output;
...
List bubbling(string input){
    List output;
    stack<class Pair> recurStack;
    Pair init, init2, stacktop;
    init.fixed_str = input[0];
    init.in_str = input.substr(1);
    init2.fixed_str = input[1];
    init2.in_str = input[0]+input.substr(2);
    recurStack.push(init);
    recurStack.push(init2);
    while(!recurStack.empty()){
        stacktop=recurStack.top();
        recurStack.pop();
        for(int i=0; i < stacktop.in_str.length()-1; i++){
            string temp_word=stacktop.in_str;
            swap(temp_word[i],temp_word[i+1]);
            Pair to_push;
            to_push.fixed_str=stacktop.fixed_str+temp_word.substr(0,i+1);
            to_push.in_str=temp_word.substr(i+1);
            recurStack.push(to_push);
        }
        output.insert(stacktop.fixed_str+stacktop.in_str);
    }
    return output;
}
