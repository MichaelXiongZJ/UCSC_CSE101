...
int main(int argc, char** argv)
{
    ...
    while(getline(input,command)) // get next line of input, store as repeat
    {
        ...
        //student test
        if(strcmp(op,"deleteKthNode") == 0){
            cout << "Delete the 2nd "+to_string(val) << endl;
            myList.deleteKthNode(val, 2);
        }
        if(strcmp(op,"deleteLastOccur") == 0){
            cout << "Delete last occurance of "+to_string(val) << endl;
            myList.deleteLastOccur(val);
        }
        if(strcmp(op,"deleteAllButFirstOccur") == 0){
            cout << "Delete all but first occurance of "+to_string(val) << endl;
            myList.deleteAllButFirstOccur(val);
        }
        if(strcmp(op,"rotate") == 0){
            cout << "Rotate by "+to_string(val) << endl;
            myList.rotate(val);
        }
